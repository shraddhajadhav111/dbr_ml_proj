...
The databricks CLI bundles top level is defined by file `house_pr_pred/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`house_pr_pred/resources/model-workflow-resource.yml`)
 - batch inference workflow (`house_pr_pred/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`house_pr_pred/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`house_pr_pred/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`house_pr_pred/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `house_pr_pred/databricks.yml` and `house_pr_pred/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-house_pr_pred-model     | /dev-house_pr_pred-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest input_default_branch content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-house_pr_pred-model | /staging-house_pr_pred-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest input_release_branch content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-house_pr_pred-model    | /prod-house_pr_pred-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the input_default_branch branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-house_pr_pred-model    | /test-house_pr_pred-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against input_default_branch branch.
...
Upon merging a PR to the input_default_branch branch, the input_default_branch branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
2. Complete the `TODO` in `house_pr_pred/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Open a pull request to merge the pushed branch into the `input_default_branch` branch.
...
Once CI passes, merge the PR into the `input_default_branch` branch. This will deploy an initial set of Databricks resources to the staging workspace.
resources will be deployed to the prod workspace on pushing code to the `input_release_branch` branch.
...
Following this, update the batch_inference_job base parameters in `house_pr_pred/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `house_pr_pred/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-house_pr_pred-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-house_pr_pred-batch-inference-job`
that runs the notebook under `house_pr_pred/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-house_pr_pred-batch-inference-job`.
...
`test-house_pr_pred-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-house_pr_pred-batch-inference-job`.
...
After setting up CI/CD, PRs against the input_default_branch branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the input_default_branch branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from input_default_branch to input_release_branch. Continuous deployment automation will deploy `prod` resources to the prod workspace.
